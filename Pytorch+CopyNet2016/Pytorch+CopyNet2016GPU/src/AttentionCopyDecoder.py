...
class AttentionCopyDecoder(nn.Module):
    ...
    def forward(self,input,pre_hidden,Encoder_outputs,sourceInput,flag=True,epsilon=1e-8):
        ...
        SelectiveReadM = torch.zeros((batch_size,1,self.hidden_size*2),dtype=torch.float32,requires_grad=True).cuda()
        ...
        for i in range(batch_size):
            ...
            mask[sourceInput[i] == inputID] = torch.tensor(1,dtype=torch.float32).cuda()
            # 保留相应位置为当前输入ID的注意力权重
        ...
        # 存储source端各个位置的累积的copy概率,其形状要与Gen_output一致，便于两者相加
        # [batch_size,output_size]
        source_position_prob = torch.tensor([[0] * self.output_size] * batch_size, dtype=torch.float32).cuda()
        # 遍历每条数据
        for i in range(batch_size):
            # 当前输入序列的ID序列
