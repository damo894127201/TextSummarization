...
class Seq2Seq(nn.Module):
    ...
    def __init__(self,config):
        ...
        self.Decoder = NNLMDecoder(output_size=config.output_size,vocab_size=config.vocab_size,embedding_size=config.embedding_size,
                                   hidden_size=config.hidden_size,C_size=config.C_size)
    ...
    def forward(self,source,source_lens,target,teacher_forcing_ratio=0.5):
        ...
        outputs = torch.zeros(target_len,batch_size,self.output_size) # [target_len,batch_size,output_size]
        # 输入解码起始符_GO开始解码,需要给batch中每条数据都输入
        GO_token = torch.tensor([[1]]*batch_size) # [batch_size,1]
        decoder_outputs = GO_token # 存储解码的单词ID结果
        # 解码长度为 batch中的序列长度
        for step in range(target_len):
            output = self.Decoder(context_vectors,decoder_outputs) # [batch_size,output_size]
            ...
            current_decoder_output = current_decoder_output.view(-1,1) # [batch_size,1]
    ...
    def BatchSample(self,source,source_lens):
        ...
        for i in range(batch_size):
            ...
            for j in range(self.MAX_LENGTH):
                ...
                output = self.Decoder(current_context_vectors,decoder_outputs) # [1,output_size]
                topv,topi = output.topk(1) # [1]
                result.append(topi.item())
                decoder_output = topi.view(1,1) # [1,1]
