...
from AttentionCopyCoverageDecoder import AttentionCopyCoverageDecoder
import torch
...
class Seq2Seq(nn.Module):
    ...
    def __init__(self,config):
        ...
        self.Decoder = AttentionCopyCoverageDecoder(embedding_size=config.embedding_size,hidden_size=config.hidden_size,
    ...
    def forward(self,source,source_lens,target,teacher_forcing_ratio=0.5):
        ...
        CoverageVector = torch.zeros((batch_size,1,max_time_step),dtype=torch.float32).cuda()
        # Coverage损失
        CoverageLoss = torch.tensor(0,dtype=torch.float32).cuda()
        ...
        for step in range(target_len):
            output, hidden, attention_weights,copy_weights,CoverageVector,Coverageloss = self.Decoder(decoder_input,hidden,Encoder_outputs,source,CoverageVector,flag=True)
        ...
        CoverageLoss = Variable(CoverageLoss,requires_grad=True).cuda() # Coverage 损失
    ...
    def BatchSample(self,source,source_lens):
        ...
        # hiddens:[2,batch_size,hidden_size*2]
        ...
        for i in range(batch_size):
            ...
            # 解码起始符
            decoder_input = GO_token
            hidden = hiddens[:,i,:] # 当前数据对应的Encoder最后时刻隐藏状态,[2,hidden_size*2]
            Encoder_output = Encoder_outputs[0][i] # 当前数据对应的Encoder各个时刻的输出
            # 由于是单条数据，因此各个维度必须要符合Decoder的输入
            decoder_input = decoder_input.unsqueeze(0)
            hidden = hidden.unsqueeze(1) # [2,1,hidden_size*2]
            ...
            CoverageVector = torch.zeros((1, 1, max_time_step),dtype=torch.float32).cuda()

            for j in range(self.MAX_LENGTH):
                # output: [1,output_size]
                output, hidden, attention_weight,copy_weight,CoverageVector,Coverageloss = self.Decoder(decoder_input,hidden,Encoder_output,source,CoverageVector,flag=False)
