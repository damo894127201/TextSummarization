...
class Encoder(nn.Module):
    def __init__(self,embedding_size,hidden_size,Embedding_table):
        ...
        self.BiGRU = nn.GRU(input_size=embedding_size,hidden_size=hidden_size,num_layers=2,batch_first=False,bidirectional=True)
    ...
    def forward(self,input,input_lens):
        ...
        # hidden: [2*2,batch_size,hidden_size]
        ...
        # 将前后两个方向最后时刻的隐状态拼接在一起，作为解码器的隐状态初始值,由于是双层GRU，因此我们让第一个维度为2
        # 表示只是将各层的前后向最后时刻的隐状态分别拼接，层与层之间不拼接；第一个维度为2仅是为了输入解码器GRU时方便
        hidden = hidden.view(2,-1,self.hidden_size*2) # [2,batch_size,hidden_size*2]
    ...
    def initHidden(self,batch_size):
        ...
        init_hidden = Variable(torch.zeros(4,batch_size,self.hidden_size)).cuda()
        return init_hidden
