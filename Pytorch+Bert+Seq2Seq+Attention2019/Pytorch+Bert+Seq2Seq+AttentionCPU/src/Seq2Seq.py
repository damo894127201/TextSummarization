...
class Seq2Seq(nn.Module):
    ...
    def __init__(self,config,BertModel):
        ...
        # 初始化encoder和decoder
        self.Encoder = Encoder(embedding_size=config.embedding_size,hidden_size=config.hidden_size,BertModel=BertModel)
        self.Decoder = AttentionDecoder(embedding_size=config.embedding_size,hidden_size=config.hidden_size,
                                        output_size=config.output_size,BertModel=BertModel,keep_prob=config.keep_prob)
    ...
    def forward(self,source,source_lens,target,teacher_forcing_ratio=0.5):
        ...
        for step in range(target_len):
            output, hidden, attention_weights = self.Decoder(decoder_input,hidden,Encoder_outputs,flag=True)
    ...
    def BatchSample(self,source,source_lens):
        ...
        for i in range(batch_size):
            ...
            for j in range(self.MAX_LENGTH):
                ...
                output, hidden, attention_weight = self.Decoder(decoder_input, hidden,Encoder_output,flag=False)
