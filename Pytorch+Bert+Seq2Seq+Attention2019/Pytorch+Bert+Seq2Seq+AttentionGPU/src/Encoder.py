...
class Encoder(nn.Module):
    def __init__(self,embedding_size,hidden_size,BertModel):
        super(Encoder,self).__init__()
        self.hidden_size = hidden_size # Encoder的RNN计算单元维度
        self.BertModel = BertModel # BertModel用于获取单词词向量
    ...
    def forward(self,input,input_lens):
        ...
        embeddings = self.BertModel(input)[0].permute(1,0,2)
        embeddings = embeddings.detach() # 截断梯度
        ...
        hidden = hidden.view(-1,1,self.hidden_size*2).contiguous() # [batch_size,1,hidden_size*2]
