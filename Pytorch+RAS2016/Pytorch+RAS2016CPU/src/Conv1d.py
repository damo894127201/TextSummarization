...
def Conv1d(input_embeddings,input_lens,B_matrix,q_size):
    ...
    outputs = torch.zeros((batch_size,max_time_step,num_filters)) # 卷积结果
    ...
    pre_start_position = torch.zeros((batch_size,pre_spaces,embedding_size),dtype=torch.float32)
    # [batch_size,end_spaces,embedding_size]
    end_position = torch.zeros((batch_size,end_spaces,embedding_size),dtype=torch.float32)
    ...
    for current_position in range(pre_spaces,pre_spaces + max_time_step):
        ...
        ConvResults = torch.zeros((batch_size,num_filters)) # [batch_size,num_filters]
    ...
    mask_vectors = torch.zeros((batch_size,max_time_step,num_filters))# mask向量
    # 遍历每条数据
    for i in range(batch_size):
        # 遍历每条数据每个实际的值，出去padding位
        for j in range(input_lens[i]):
            mask_vectors[i,j] += torch.ones((num_filters))
    # mask掉那些填充位
