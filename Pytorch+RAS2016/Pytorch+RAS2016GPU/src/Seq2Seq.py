...
class Seq2Seq(nn.Module):
    ...
    def __init__(self,config):
        ...
        self.num_filters = config.embedding_size # 卷积核的个数
        self.B_matrix = Variable(torch.Tensor(self.num_filters,config.q_size,config.embedding_size).uniform_(-1,1),requires_grad=True).cuda() # 初始化B矩阵，公式(2)
        # 初始化encoder和decoder
        self.Encoder = Encoder(embedding_size=config.embedding_size,hidden_size=config.hidden_size,
                               Embedding_table=self.Embedding_table,B_matrix=self.B_matrix,q_size=config.q_size)
        self.Decoder = AttentionDecoder(embedding_size=config.embedding_size,hidden_size=config.hidden_size,
    ...
    def forward(self,source,source_lens,target,teacher_forcing_ratio=0.5):
        ...
        # Encoder
        Encoder_outputs,hidden,cell_state = self.Encoder(source,source_lens)
        ...
        for step in range(target_len):
            output, hidden,cell_state,attention_weights = self.Decoder(decoder_input,hidden,cell_state,Encoder_outputs,flag=True)
    ...
    def BatchSample(self,source,source_lens):
        ...
        # cell_states: [1,batch_size,hidden_size*2]
        Encoder_outputs, hiddens,cell_states = self.Encoder(source, source_lens)
        ...
        for i in range(batch_size):
            ...
            # 解码起始符
            decoder_input = GO_token
            hidden = hiddens[i] # 当前数据对应的Encoder最后时刻隐藏状态,[1,hidden_size*2]
            cell_state = cell_states[0][i] # 当前数据对应的Encoder最后时刻的细胞状态,[hidden_size*2]
            Encoder_output = Encoder_outputs[0][i] # 当前数据对应的Encoder各个时刻的输出,[1,hidden_size*2]
            # 由于是单条数据，因此各个维度必须要符合Decoder的输入
            decoder_input = decoder_input.unsqueeze(0)
            hidden = hidden.unsqueeze(0)
            cell_state = cell_state.view(1,1,-1).contiguous() # [1,1,hidden_size*2]
            Encoder_output = (Encoder_output.unsqueeze(0),0) # 将Encoder_output转为二元元组，以符合Decoder的输入
            for j in range(self.MAX_LENGTH):
                # output: [1,output_size]
                output, hidden,cell_state,attention_weight = self.Decoder(decoder_input,hidden,cell_state,Encoder_output,flag=False)
