...
class Encoder(nn.Module):
    ...
    def forward(self,input,input_lens,target,target_lens,batch_size,descendingOrders,input_article_lens,flag):
        ...
        sentenceEncoding = sentenceEncoding.view(batch_size,-1,self.hidden_size*2).contiguous() # [batch_size,max_sent_num,hidden_size*2]
        ...
        if flag == 'Train':
            ...
            epsilon = torch.rand_like(recog_std).cuda()
            ...
            epsilon = torch.rand_like(prior_std).cuda()
            # Reparameterization Trick
