...
if __name__ == '__main__':
    ...
    loader = DataLoader(dataset=MyDataSet(source_path, target_path, tokenizer), batch_size=config.batch_size, shuffle=True,
                        num_workers=0,collate_fn=pad,drop_last=False) # 最后一个batch数据集不丢弃
    # 评估集
    eval_loader = DataLoader(dataset=MyDataSet(eval_source_path, eval_target_path, tokenizer),batch_size=config.batch_size,
                             shuffle=True, num_workers=0, collate_fn=pad, drop_last=False)  # 最后一个batch数据集不丢弃
    ...
    KL_weights = torch.linspace(0,1,config.KL_cost_annealing_step) # 在前config.KL_cost_annealing_step个step内，每个step
                                                                   # 的KL损失的权重，如果当前step大于等于config.KL_cost_annealing_step
                                                                   # 则该step的权重为1
    ...
    for epoch in range(config.num_Epochs):
        for iter,(batch_x,batch_y,batch_source_lens,batch_target_lens,batch_size,descendingOrders,source_article_lens) in enumerate(loader):
            optimizer.zero_grad() # 每批次训练时初始梯度得置0
            batch_loss = 0 # 每批次数据的损失
            bow_loss = 0 # 每批次的bow损失
            target_max_length = batch_y.size(1)
            # outputs: [max_time_step,batch_size,vocab_size],KL_loss为KL散度损失
            # 由于是kgCVAE，因此target也是输入batch_y
            batch_x = batch_x.cuda()
            batch_source_lens = torch.as_tensor(batch_source_lens).cuda()
            batch_y_cuda = batch_y.cuda()
            batch_target_lens_cuda = torch.as_tensor(batch_target_lens).cuda()
            descendingOrders = torch.as_tensor(descendingOrders).cuda()
            source_article_lens = torch.as_tensor(source_article_lens).cuda()
            outputs,KL_loss,mlp_outputs = model(batch_x,batch_source_lens,batch_y_cuda,batch_target_lens_cuda,batch_size,descendingOrders,source_article_lens,config.TeacherForcingRate)
            # 处理PAD填充位，避免对其进行损失计算,CrossEntropy
            outputs = sequence_mask_loss(outputs,batch_target_lens,config.output_size)
            # 计算BOW损失
            mlp_outputs = sequence_mask_loss(mlp_outputs,batch_target_lens,config.output_size)
            ...
            for j in range(target_max_length):
                ...
                bow_loss += criterion(mlp_outputs[j], batch_y[j])

            # 将交叉熵损失和bow损失累加
            batch_loss = batch_loss + bow_loss
            KL_loss = KL_loss.cpu()
            # KL cost annealing
            if step >= config.KL_cost_annealing_step :
                batch_loss = batch_loss + 1 * KL_loss
            else:
                batch_loss = batch_loss + KL_weights[step] * KL_loss
            # 由论文《Auto-Encoding Variational Bayes》可知，我们要最小化D(q(z|x)||p(z|x)),等价于最大化ELBO，即这里的batch_loss
            # ELBO = CrossEntropyLoss(Reconstruction quality) - w * D(q(z|x)||p(z))
            # 而 D(q(z|x)||p(z|x)) = logp(x) - ELBO >= 0
            # 注意p(z|x)与p(z)不同，在实际中，我们用标准正态分布来表示p(z)，模型里用两个线性层学习q(z|x)的均值和log方差，
            # 故而，我们假设变分分布q(z|x)服从正态分布，p(z|x)是未知的
            ...
            if step % print_every == 0:
                ...
                batch_eval_rouge2 = []
                batch_eval_rougeL = []
                for eval_iter,(batch_eval_x,batch_eval_y,batch_eval_source_lens,batch_eval_target_lens,batch_eval_size,eval_descendingOrders,eval_source_article_lens) in enumerate(eval_loader):
                    batch_eval_x = batch_eval_x.cuda()
                    batch_eval_source_lens = torch.as_tensor(batch_eval_source_lens).cuda()
                    eval_descendingOrders = torch.as_tensor(eval_descendingOrders).cuda()
                    eval_source_article_lens = torch.as_tensor(eval_source_article_lens).cuda()
                    eval_outputs = model.BatchSample(batch_eval_x,batch_eval_source_lens,batch_eval_size,eval_descendingOrders,eval_source_article_lens)
                    # 随机输出一条预测结果
                    '''
            ...
            if step > max(num_training_steps,config.KL_cost_annealing_step) and len(RougeL) > num_eval:
