...
class NNLMDecoder(nn.Module):
    ...
    def forward(self,encoder_output,predict_sequence,source,source_lens):
        ...
        if actual_len < self.C_size:
            padding_contexts = torch.zeros((batch_size, self.C_size - actual_len, self.embedding_size)).cuda()
            C_size_contexts = torch.cat((C_size_contexts, padding_contexts), 1)

        # 变换C_size_contexts的维度
        C_size_contexts = C_size_contexts.view(batch_size,-1).contiguous()  # [batch_size,C_size*embedding_size]
        ...
        unigram_feature = torch.zeros((batch_size, self.output_size)).cuda()  # [batch_size,output_size]
        bigram_feature = torch.zeros((batch_size, self.output_size)).cuda()  # [batch_size,output_size]
        trigram_feature = torch.zeros((batch_size, self.output_size)).cuda()  # [batch_size,output_size]
        reordering_feature = torch.zeros((batch_size, self.output_size)).cuda()  # [batch_size,output_size]
