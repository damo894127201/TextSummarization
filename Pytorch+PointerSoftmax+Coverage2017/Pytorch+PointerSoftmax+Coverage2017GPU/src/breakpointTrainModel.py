...
from PointerGeneratorCoverage import PointerGeneratorCoverage
...
if __name__ == '__main__':
    ...
    pre_trainModel = '../model/model.pth20000'
    ...
    for epoch in range(startEpoch,config.num_Epochs):
        for iter,(batch_x,batch_y,batch_source_lens,batch_target_lens) in enumerate(loader):
            ...
            batch_source_lens_cuda = torch.as_tensor(batch_source_lens).cuda()
            batch_y_cuda = batch_y.cuda()
            # outputs: [max_time_step,batch_size,vocab_size]
            outputs,CoverageLoss = model(batch_x_cuda,batch_source_lens_cuda,batch_y_cuda,config.TeacherForcingRate)
            # 处理PAD填充位，避免对其进行损失计算
            ...
            # 训练损失= 交叉熵损失 + ratio * CoverageLoss
            batch_loss = batch_loss + config.ratio * CoverageLoss
            ...
            if step % print_every == 0:
                ...
                for eval_iter, (batch_eval_x, batch_eval_y, batch_eval_source_lens, batch_eval_target_lens) in enumerate(eval_loader):
                    batch_eval_x_cuda = batch_eval_x.cuda()
                    batch_eval_source_lens_cuda = torch.as_tensor(batch_eval_source_lens).cuda()
                    eval_outputs, _ = model.BatchSample(batch_eval_x_cuda, batch_eval_source_lens_cuda)
                    # 随机输出一条预测结果
                    '''
